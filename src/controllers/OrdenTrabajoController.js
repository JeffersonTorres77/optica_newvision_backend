...
const OrdenTrabajoController = {
    ...
    change_status: async (req, res) => {
        ...
        else if (estado === 'listo_laboratorio') progreso = 90;
    ...
    update_process: async (req, res) => {
        ...
        const obj_orden_trabajo = await OrdenTrabajo.findOne({ where: { orden_key: orden_numero } });
        if (!obj_orden_trabajo) {
            throw { message: `Orden ${orden_numero} no encontrada` };
        }

        if (!VerificationUtils.verify_numero(progreso) || progreso < 0 || progreso > 100) {
            throw { message: `El progreso debe ser un numero entre 0 y 100: ${progreso}` };
        }

        obj_orden_trabajo.progreso = progreso;
        await obj_orden_trabajo.save();

        const orden_output = (await OrdenTrabajoService.formatear_get_orden_trabajo([obj_orden_trabajo.id]))[0];
        res.status(200).json({ message: 'ok', ordenes_trabajo: orden_output });
    },
